...
inline long double get_IBR(long double N,long double R,long double B,long double phi,long double lam,long double om, long double* s_sin_H,long double* s_sin_fi,long double* s_IbBR){
    om = ((om) * M_PI)/180;
    long double X = f_X(N);
//    long double E = f_E(X);
...
void solve(long double phi, long double lam,long double ans[]){
    ...
    for(int R = 0; R <= 0; R++){
        for(int B = 45; B <= 45; B++){
            for(int q = 0; q <= 3; q++){
                for(int i = -75; i <= 75; i += 15){
                    long double sin_H,sin_fi,IbBR;
//                    cout << "debug3:" << N[q] << ' ' << R << ' ' << B << ' ' << i << endl;
                    long double IBR = get_IBR(N[q],((R) * M_PI)/180,((B) * M_PI)/180,phi,lam,i,&sin_H,&sin_fi,&IbBR);
                    pii tmp = is_out(sin_H,sin_fi);
//                    cout << "debug4:" << IbBR << ' ' << tmp.F << ' ' << tmp.S << endl;
                    cout << IbBR * tmp.F <<',';
                    ans[B] += IBR * tmp.F;
